...
**Method signature:**
- **`await ctx.emit_event(topic, payload=None, *, priority="normal", source=None, expires_at=None, event_id=None, retained=None)`**: Publish an event to matching subscribers. See [Events](/servers/events) for full parameter documentation.
